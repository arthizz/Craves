						...
						<div class="payments-methods">
							...
							<div class="input-checkbox">
								<label for="payments-1">Cash On Delivery</label>
									...
									<p>Cash on delivery (COD), sometimes called collect on delivery, is the sale of goods by mail order where payment is made on delivery rather than in advance. If the goods are not paid for, they are returned to the retailer. Originally, the term applied only to payment by cash but as other forms of payment have become more common, the word "cash" has sometimes been replaced with the word "collect" to include transactions by checks, credit cards or debit cards.</p>
					...
					<div class="col-md-12">
						<div class="order-summary clearfix">
							...
							<table class="shopping-cart-table table">
								...
								<tbody>
									<?php
									$stmt2 = $conn->prepare("SELECT DISTINCT cPrice FROM cart_tbl Where user_id = :id");
									$stmt2->bindparam(":id", $_SESSION['user']);
									$stmt2->execute();
									$total_all = 0;

									while($row1 = $stmt2->fetch(PDO::FETCH_ASSOC)){
											$stmt3 = $conn->prepare("SELECT * FROM cart_tbl Where cPrice = :product AND user_id = :id");
									...
									?>
									<tr>
										<td class="thumb"><img src="./img/<?=$row2['cProduct_img']?>" alt=""></td>
										<td class="details">
									...
									<!-- Modal -->
									<div class="modal fade" id="<?=$row2['cID']?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
									        ...
									        <div class="container-fluid">
									        	...
									        	<div class="row">
									        		<div class="col-md-6">
									        			<div class="product product-widget">
															<div class="product-thumb">
																<img src="img/<?=$row2['cProduct_img']?>" alt="">
															</div>
															<div class="product-body">
																<h3 class="product-price">₱<?=$row2['cPrice']?>.00
																<h2 class="product-name"><a href="#"><?=$row2['cProduct']?></a></h2>
															</div>
														</div>
									        		</div>
									        		<div class="col-md-6" style="margin-top: 10px;">
									        				<form method="post" action="api/delete-cart.php?name=<?=$row2['cProduct']?>&&price=<?=$row2['cPrice']?>" >
									        				  <div class="row">
									        				  	<div class="col-md-6">
																  <div class="form-group">
																    <label>Quantity</label>
																    <input type="number" value="<?php echo $count;?>" min="1" max="<?php echo $count;?>" class="form-control" name="qty">
																  </div>
															  	</div>
							...
							<div class="pull-right">
								<a href="api/checkout-process.php?id=<?=$row2['user_id']?>"><button class="primary-btn">Place Order</button></a>
							</div>
						</div>
