...
<body>
            ...
            <div class="col-sm-4">
                <div class="page-header float-left">
                    <div class="page-title">
                        <h1>Monthly Sales and Expenses</h1>
                    </div>
                </div>
            </div>
            <div class="col-sm-8">
            ...
            <div class="row">
                ...
                <div class="col-md-5 mx-auto mt-5">
                    ...
                    <table class="table">
                      ...
                      <tbody>
                        <?php
                        $stmt = $conn->prepare("SELECT `cProduct`, COUNT(`cProduct`) AS `value_occurrence` FROM `cashier_tbl` GROUP BY `cProduct` ORDER BY `value_occurrence` DESC LIMIT 1");
                        $stmt->execute();
    ...
    $revenues = '';
    $dates = '';
    $sth = $conn->prepare("SELECT * FROM chart_tbl");
    $sth->execute();

    while($row = $sth->fetch(PDO::FETCH_ASSOC)){
        $expense = $row['cExpenses'];
        $revenue = $row['cRevenue'];
        $date = date('M-Y', strtotime($row['cBudget_date']));
    ...
    <script type="text/javascript">
        ...
        var data = {
            datasets: [{
                ...
                backgroundColor: 'rgba(255, 120, 115, 0.5)',
                borderColor: "#FF0000",
                borderWidth: "5",
                label: 'Expenses',
            }],
