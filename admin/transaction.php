        ...
        <div class="content mt-3">
            <div class="wala col-md-12">
                <table class="table">
                  ...
                  <tbody> 
                    <?php
                    $stmt = $conn->prepare("SELECT DISTINCT user_id FROM checkout_tbl");
                    $stmt->execute();
                    

                        while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                            $stmt1 = $conn->prepare("SELECT * FROM checkout_tbl Where user_id = :id");
                            $stmt1->bindparam(":id", $row['user_id']);
                            $stmt1->execute();
                            $count = $stmt1->rowCount();
                            $row1 = $stmt1->fetch(PDO::FETCH_ASSOC);

                            $stmt2 = $conn->prepare("SELECT * FROM user_tbl WHERE cID = :id");
                            $stmt2->bindparam(":id", $row['user_id']);
                            $stmt2->execute();
                    ...
                    if($row1['cStatus'] == 0){
                    ?>
                        <button class="btn btn-success btn-xs" onClick="window.open('../api/updatetrans.php?id=<?=$row1['user_id']?>', '_self',); window.close();" style="border-radius: 5px;"><i class="fas fa-check"></i></button>
                        <!-- <button class="btn btn-primary btn-xs" style="border-radius: 5px;" onclick="window.print();"><i class="fas fa-print"></i></button> -->
                    <?php
                }else if($row1['cStatus'] == 1){

                    ?>

                     <button class="btn btn-success btn-xs" onClick="window.open('../api/updatetrans.php?id=<?=$row1['user_id']?>', '_self',); window.close();" style="border-radius: 5px;">On Going</button>
                     <?php
                 }else{
                     ?>
                                    ...
                                    <table class="shopping-cart-table table">
                                        ...
                                        <tbody>
                                            ...
                                            while($row1 = $stmt2->fetch(PDO::FETCH_ASSOC)){
                                                    ...
                                                    $stmt3->bindparam(":id", $row2['user_id']);
